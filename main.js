...
function abbreviationName(character) {
...
function swapCase(character) {
     ...
     * @param : motTable -> tableau qui contient les mots de notre chaine
     * @param : TableFormat -> tableau contenant les mots reformatter
     */

    let motTable = character.split(" ");
    let TableFormat = [];

    for(let word of motTable) {
        if(word.charAt(0) == word.charAt(0).toUpperCase()) {

            word = word.charAt(0).toLowerCase() + word.substring(1).toUpperCase();
            TableFormat.push(word);
